...
def test_ucatch(method, cellsize):
    ...
    # upstream
    rivlen, rivslp = ucat.channel(
        idxs_out, idxs_us_main, elv, upa, ncol, latlon=True, mv=mv
    )
    if cellsize == 1:
    ...
    # downstream
    rivlen1, rivslp1 = ucat.channel(
        idxs_out, idxs_ds, elv, upa, ncol, latlon=True, mv=mv
    )
