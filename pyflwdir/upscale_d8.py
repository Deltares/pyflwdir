...
def upscale_d8(scale_ratio, flwdir, uparea, upa_min=0.5, method='extended'):
    """Returns upscaled flow direction map 
    defulat method: extended effective area method (Eilander et al., in progress) 
    
    Arguments:
    ----------
    scale_ratio: int 
        The upscaling ratio
    flwdir: :obj:`numpy.ndarray`
        Local flow direction map. 
    uparea: :obj:`numpy.ndarray`
        Upstream area map.
    upa_min: float
        Minimum upstream area for rivers 
        Default: 0.5
    method: {'extended'} 
        Upscaling method (default: {'extended'})
    

    Returns:
    --------
    Upscaled flow direction map: :obj:`numpy.ndarray`
    """
...
def _outlet(idx0, subidx, flwdir_flat, shape, scale_ratio):
    ...
    streamcells = [subidx]
    while True:
        subidx_ds = d8.ds_index(subidx, flwdir_flat, shape) # move downstream
...
def _dd(idx0, subidx, flwdir_flat, effare_flat, shape, shape_lr, scale_ratio, extended=True):
    ...
    while True:
        ...
        # in neighboring lowres cell
            ...
            if  flag >= np.uint8(1):
                if idx_ds0 != idx_ds:
                    dd = d8.idx_to_dd(idx0, idx_ds, shape_lr)
                    idx_ds0 = idx_ds
                    # assert dd != _mv
...
def _force_ds_outlet(idx0, subidx0, scale_ratio, flwdir_flat, effare_flat, shape, shape_lr, n):
    ...
    if ea != np.uint8(0):
        ...
        while j < 3:
            ...
            dd = d8.idx_to_dd(idx0, idx_ds, shape_lr)
            path.extend(path2)
            if dd != _mv and d8.ispit(dd) == False:
                if i == n:
                    idx_ds1 = idx_ds
                    subidx1 = subidx
                    dd1 = dd
                ...
                j += 1
            subidx_ds = d8.ds_index(subidx, flwdir_flat, shape)
...
def _ddext(idx0, flwdir_lr_flat, outlet_lr_flat, checkd_lr_flat, 
    ...
    while True:
        ...
        while not success and i < 5:
            if idx_ds1 == -1 or d8.ispit(flwdir_lr_flat[idx_ds1]): break #  or checkd_lr_flat[idx_ds1] == 1
            # get original ds outlet location
            idx_ds0 = d8.ds_index(idx1, flwdir_lr_flat, shape_lr)
            ...
            subidx = subidx1
            nn = 1
            while idx_ds1 in idx_path + bottleneck or d8.ispit(flwdir_lr_flat[idx_ds1]): #  or checkd_lr_flat[idx_ds1] == 1
                idx_ds1, subidx1, dd1, path1 = _force_ds_outlet(
        ...
        if success:
            ...
            for i in range(len(path)):
                ...
                if idx != idx1:
                    ...
                    if idx in idx_path or d8.ispit(flwdir_lr_flat[idx_ds1]): #  or checkd_lr_flat[idx_ds1] == 1
                        continue
                    ...
                    idx1 = idx
                if branch:
                    for subidx_us in d8.us_indices(subidx, flwdir_flat, shape):
        ...
        if success:
            for idx_ds in idx_path:
                ...
                for idx_us in d8.us_indices(idx_ds, flwdir_lr_flat, shape_lr):
                    ...
                    dd1 = d8.idx_to_dd(idx_us, idx1, shape_lr)
                    if dd1 == _mv or d8.ispit(dd1):           
                        success = False
        ...
        if success: 
            ...
            while loop == False:
                if idx1 in idx0_lst:
                    ...
                    idx_ds1 = d8.ds_index(idx1, flwdir_lr_flat, shape_lr, dd=dd1)
                else:
                    idx_ds1 = d8.ds_index(idx1, flwdir_lr_flat, shape_lr)
...
def _ddplus(idx0, flwdir_lr_flat, outlet_lr_flat, flwdir_flat, uparea_flat, effare_flat, 
    ...
    idxs_us = d8.us_indices(idx0, flwdir_lr_flat, shape_lr)
    idx_ds = d8.ds_index(idx0, flwdir_lr_flat, shape_lr)
    if idxs_us.size == 0 or idx_ds == idx0 or idx_ds < 0:
        return idx0, _mv, subidx_out
    ...
    dd_new = d8.idx_to_dd(idx_us_main, idx_ds, shape_lr)
    assert not d8.ispit(dd_new)
    if dd_new == _mv: # -1 if it does not connect in d8
        return idx0, _mv, subidx_out
    # trace main stream downstream
    subidx_us = subidxs_out_us[maxi]
    mainstream = [subidx_us]
    while True:
        subidx_ds = d8.ds_index(subidx_us, flwdir_flat, shape)
    ...
    while True and j < n-1:
        ...
        subidxs_us = d8.us_indices(subidx_ds, flwdir_flat, shape)
...
@njit
def _upscale_d8(scale_ratio, flwdir_flat, uparea_flat, shape, upa_min=0.5, extended=True):
    ...
    for idx0 in range(flwdir_lr_flat.size):
        ...
        if dd != _mv and effare_flat[subidx_out] <= np.uint(1): # subgrid outlets might not be connected
            ...
            if  d8.idx_to_dd(idx0, idx_ds, shape_lr) != dd: # not connected
                idx_check.append(idx0)
                upa_check.append(uparea_flat[subidx0])
                changd_lr_flat[idx0] = np.uint(1)
        if subidx_out != -1 and d8.ispit(dd) and d8.ispit(flwdir_flat[subidx0]) == False: # keep list of cells with outlet == pits outside own cell
            assert d8.ispit(flwdir_flat[subidx_out])
            ...
            if i >= j and subidxs_sort[i] == subidx: # if new double pit
                ...
                for k in range(idxs0.size):
                    ...
                    if k == imax: # set pit to cell with largest upstream area
                        ...
                        dd = d8.idx_to_dd(idx, idx_out0, shape_lr)
                        assert dd != _mv # make sure cells connect in d8
    ...
    if extended:
        for idx0 in range(flwdir_lr_flat.size):
            if flwdir_lr_flat[idx0] == _mv: continue
    ...
    # assert flwdir_check(flwdir_lr_flat, shape_lr)[1] == False # no loops  #NOTE works for 
