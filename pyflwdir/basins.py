...
@njit
def interbasin_mask(idxs_ds, seq, region_mask, stream_mask=None):
    ...
    if stream_mask is not None:
        ...
        mask = np.array([bool(0) for _ in range(region_mask.size)])  # all True
    ...
    for idx in seq:  # down- to upstream
        ...
        if region_mask[idx] == False and region_mask[idx_ds]:  # leaving region
            mask[idx] = False
        # elif region_mask[idx_ds] == False and region_mask[idx] and mask[idx_ds]:
        #     idxs_out.append(idx)
    return np.logical_and(
        mask, region_mask
    )  # , np.array(idxs_out, dtype=idxs_ds.dtype)


# @njit
# def headwater_mask(idxs_ds, seq, region_mask, stream_mask=None):
